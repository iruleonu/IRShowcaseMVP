//
//  DataProviderLocalThenRemoteTests.swift
//  IRShowcaseMVP
//
...
@Suite(.tags(.dataProvider, .localFirstThenRemoteDataProviderConfig))
final class DataProviderLocalThenRemoteTests {
    let network = APIServiceMock()
    let persistence = PersistenceLayerMock()
    let dpConfig = DataProviderConfiguration.localFirstThenRemote
