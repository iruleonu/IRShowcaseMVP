//
//  PersistenceLayer+SaveExampleData.swift
//  IRShowcaseMVP
//
...
extension PersistenceLayerImpl {
    func saveStaticElemsToJSON() {
        let elemsToParse = [
        ...
        var auxArray: [BabyNamePopularity] = []
        for elem in elemsToParse {
            auxArray.append(BabyNamePopularity(yearOfBirth: Int(elem[0])!, gender: Gender(rawValue: elem[1]) ?? .unknown, ethnicity: elem[2], name: elem[3], numberOfBabiesWithSameName: Int(elem[4])!, nameRank: Int(elem[5])!))
        }
        JSONEncoder.encode(from: auxArray)
    }
}
